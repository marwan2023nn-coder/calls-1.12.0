...
func (p *Plugin) handleClientMsg(us *session, msg clientMessage, handlerID string) error {
	...
	case clientMessageTypeRemoteControl:
		...
		sharingSession, ok := state.sessions[state.Call.Props.ScreenSharingSessionID]
		if !ok {
...
func (p *Plugin) WebSocketMessageHasBeenPosted(connID, userID string, req *model.WebSocketRequest) {
	...
	case clientMessageTypeSDP:
		...
		msg.Data = data
	case clientMessageTypeICE, clientMessageTypeScreenOn, clientMessageTypeVideoOn, clientMessageTypeRemoteControl:
